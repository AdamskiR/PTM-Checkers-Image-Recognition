...
public class ReadColorWhiteinHSV : MonoBehaviour
{
    [SerializeField] string imagePath = "/CameraPictures/CameraInput.png";

    [Header("RGB Colors")]
    [SerializeField] [Range(0, 1)] float HvalueMin = 0.2f;
    [SerializeField] [Range(0, 1)] float HvalueMax = 0.3f;
    [SerializeField] [Range(0, 1)] float Svalue = 0.4f;
        ...
        if (File.Exists(Application.dataPath + imagePath))
        {
            ...
            for (int x = 0; x < newTex.width; x++)
            {
                for (int y = 0; y < newTex.height; y++)
                {
                    ...
                    if (H < HvalueMax && H > HvalueMin && S < Svalue && V > Vvalue)
                    {
                        mapa[x, y] = 1;
