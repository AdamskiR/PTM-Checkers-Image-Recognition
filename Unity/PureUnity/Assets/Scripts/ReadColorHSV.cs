...
public class ReadColorHSV : MonoBehaviour
{
    [SerializeField] string imagePath = "/CameraPictures/CameraInput.png";

    [Header("HSV Colors")]
    [SerializeField] string name;
    [SerializeField] [Range(0, 1)] float HvalueMin = 0f;
    [SerializeField] [Range(0, 1)] float HvalueMax = 1f;
    [SerializeField] [Range(0, 1)] float Svalue = 0.5f;
    [SerializeField] [Range(0, 1)] float VvalueMin = 0f;
    [SerializeField] [Range(0, 1)] float VvalueMax = 1f;
        ...
        if (File.Exists(Application.dataPath + imagePath))
        {
            ...
            for (int x = 0; x < newTex.width; x++)
            {
                for (int y = 0; y < newTex.height; y++)
                {
                    ...
                    if (H < HvalueMax && H > HvalueMin && S < Svalue && V > VvalueMin && V < VvalueMax)
                    {
                        mapa[x, y] = 1;
            ...
            //Destroy(newTex);
            //File.WriteAllBytes(Application.dataPath + "/CameraPictures/CameraInputMapWhite.png", Bytes);
